...
from soccer_eventpred.data.sources.jleague import JLeagueDataSource
...
@SoccerDataModule.register("jleague_single")
class JLeagueSingleEventDataModule(SoccerDataModule):
    def __init__(
        self,
        train_datasource: JLeagueDataSource,
        val_datasource: Optional[JLeagueDataSource] = None,
        test_datasource: Optional[JLeagueDataSource] = None,
        batch_size=32,
        num_workers=0,
            ...
            for event in event_sequence.events:
                ...
                if self._event2label is not None:
                    event_id = self.vocab.add(
                        self._event2label[event.event_name], "events"
                    )
                    self.event_counts[event_id] = self.event_counts.get(event_id, 0) + 1
                else:
                    event_id = self.vocab.add(event.event_name, "events")
                    self.event_counts[event_id] = self.event_counts.get(event_id, 0) + 1
    ...
    def _prepare_instance(self, event_sequence):
        ...
        if self._event2label is not None:
            event_ids = [
                self.vocab.get(self._event2label[event.event_name], "events")
                for event in event_sequence.events
            ]
        else:
            event_ids = [
                self.vocab.get(event.event_name, "events")
                for event in event_sequence.events
            ]
